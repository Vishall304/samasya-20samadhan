...
const testCases = [
  ...
  {
    input:
      "mujhe lagta hai ki main depression mein hun aur kuch samjh nahi aa raha",
    expected: "hinglish",
...
export function runLanguageDetectionTests() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    console.log(
      `Detected: ${result.language} (confidence: ${result.confidence.toFixed(2)})`,
    );
...
export function testResponseFormatting() {
  ...
  testInputs.forEach((test, index) => {
    ...
    const formattedResponse = formatResponseByLanguage(
      detectedLang,
      sampleResponses,
    );
